...
m=1, k=1,   % virpancios sistemos parametrai: 0.05
nu=0.1,     % slopinimo laipsnis nu=c/2/sqrt(k*m) 0.03
c=2*nu*sqrt(k*m)   

lambda=1, ampl=0.1, % kelio bangos ilgis ir amplitude
x=0; v0=0; a=0.25; % pradinis atstumas, pradinis judejimo greitis ir pastovus pagreitis  
...
dt=2*pi/(200*omrez);% zingsni parenkame maza, ivertindami, kad laikui begant  
                    % virpesiu daznis dideja, kai dideja judejimo greitis
...
ttt=[0:dt:tmax];
[TT,YY]=ode45(@fnk,ttt,y);
Y=YY(:,1);S=YY(:,3);
ttp=0;
T=[];Y=[];S=[];
for i=1:length(TT)
    y=YY(i,:);
    T=[T,TT(i)];Y=[Y,y(1)];S=[S,y(3)];
    ...
    str=sprintf('t=%8.3g, vel=%8.3g',TT(i),greitis(TT(i)));title(['\fontname{Courier}',str]);
end
